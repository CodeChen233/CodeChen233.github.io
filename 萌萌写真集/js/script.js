...
for (let i = 0; i < li.length; i++) {
  li[i].style.background = `url(./images/pinkmeng${i + 1}.jpg) no-repeat center/cover`;
}
...
// 创建元素
function creatImg(name, number,startNum) {
  // 先清空原有图片
  waterfall.innerHTML = '';
  for (let i = 0; i < number; i++) {
    ...
    img.src = `./images/${name}/${(startNum + i + 1) + '.jpg'}`;
    div.appendChild(img);
...
nav.addEventListener('click', function (e) {
  if (e.target.tagName === 'LI') {
    ...
    if (name === 'other') creatImg(name, 37, 20000);
    else if (name === 'pink') creatImg(name,23,10000);
  }
});
